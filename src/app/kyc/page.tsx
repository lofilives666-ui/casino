import Link from "next/link";
import KycFlow from "@/components/account/KycFlow";
import { requireSessionUser } from "@/lib/current-user";

export const runtime = "nodejs";

export default async function KycPage() {
  await requireSessionUser();

  return (
    <div className="casino-bg min-h-screen p-4">
      <div className="mx-auto max-w-4xl space-y-4">
        <div className="panel rounded-xl p-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="section-title text-2xl text-white">KYC Verification</h1>
              <p className="text-sm text-[#8fb1cc]">Complete verification to unlock withdrawals and higher limits.</p>
            </div>
            <div className="flex gap-2">
              <Link href="/" className="pill px-3 py-1.5 text-sm text-[#dce9ff]">
                Lobby
              </Link>
              <Link href="/wallet" className="rounded-full bg-[#2d7de0] px-4 py-1.5 text-sm font-semibold text-white">
                Wallet
              </Link>
            </div>
          </div>
        </div>

        <KycFlow />
      </div>
    </div>
  );
}

