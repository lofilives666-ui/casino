...
export async function beginIdempotentRequest(opts: {
  ...
  if (redis) {
    try {
      const existingPayload = await redis.get(responseKey);
      const existingParsed = existingPayload ? parseResponsePayload(existingPayload) : null;
      if (existingParsed) {
        return {
          ok: false,
          reason: "REPLAY",
          response: existingParsed,
          cacheKey,
          lockKey,
          responseKey,
          backend: "redis",
      ...
      const payloadAfterLockMiss = await redis.get(responseKey);
      const parsedAfterLockMiss = payloadAfterLockMiss ? parseResponsePayload(payloadAfterLockMiss) : null;
      if (parsedAfterLockMiss) {
        return {
          ok: false,
          reason: "REPLAY",
          response: parsedAfterLockMiss,
          cacheKey,
          lockKey,
          responseKey,
          backend: "redis",
